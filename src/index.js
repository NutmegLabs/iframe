...
export default class Iframe {
  ...
  save(blockContent) {
    ...
    Object.assign(this.data, { frame: iframe.parentElement.innerHTML });
  ...
  onPaste(event) {
    switch (event.type) {
      case "pattern": {
        let iframe = event.detail.data;
        const position = iframe.indexOf(">", 0);
        if (!iframe.match(/sandbox/)) {
          iframe = iframe.slice(0, position) + ' sandbox=\"allow-scripts allow-same-origin\"' + iframe.slice(position);
        }
        this.data = {
          frame: iframe,
        }
